...
.isOrganismValid<-function(organism, showWarning=TRUE){
  ...
  if((! result) & showWarning){
    warning(paste0("organism ", organism, " is not supported by WebGestalt, no pathway analysis will be performed. Valid organisms include: ", paste(validOranisms,collapse=", ")))
  }
  return(result)
}
