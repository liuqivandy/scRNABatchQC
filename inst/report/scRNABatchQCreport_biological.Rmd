...
scRNABatchQC first selects the top 50 genes with highest and significant biological variance in each experiment (FDR<0.01) and then clusters experiments based on the similarity on highly variable genes. `r getHeatmapCaption(hvgBiologicalSimilarity,"hvg")` shows the biological variance of top 50 genes in each experiment and their similarity across experiments.
...
cat("\n", getHeatmapCaption(hvgPathways,"hvgPath"), " shows the -log10 FDR for enriched pathways in HVGs in each experiment and pathway similarity across experiments.")
```
...
scRNABatchQC selects the top 50 differentially expressed genes along the specific principle component (PCind, default: the first PC) in each experiment (FDR<0.01). `r getHeatmapCaption(pc1geneBiologicalSimilarity,"PCg")` shows differential expression of top 50 PC-related genes in each experiment and genes similarity across experiments.
...
cat("\nscRNABatchQC performs the functional enrichment analysis for the PC-related genes in each experiment. ", getHeatmapCaption(pc1Pathways,"PCgPath"), " shows the -log10 FDR for enriched pathways in PC-related genes in each experiment and pathway similarity across experiments.")
```
