...
```{r plotDiffGenes, eval=drawHeatmap, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.align="center", fig.cap=figRef("plotDiffGenes", "Heatmap of differentially expressed genes in pairwise comparison", trunk.eval=drawHeatmap), fig.height=getHeatmapHeight(plotData$diffFC$genes), fig.width=10}
heatmap.2(as.matrix(plotData$diffFC$genes), cexRow = 0.6, margins=heatmapMargins, keysize=heatmapKeysize, col=heatmapColors)
```

scRNABatchQC detects differentially expressed genes between two experiments (logFC>`r plotData$logFC` & FDR<`r plotData$FDR`). 
