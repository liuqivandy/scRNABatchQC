...
plotHeatmap<-function(datam,feature=c("hvg","hvgPath","PCg","PCgPath","diffg","diffgPath")){
  ...
  if (feature== "hvg" | feature== "PCg" | feature=="diffg"){ 
            colv=heatmapColors
	} else colv=heatmapPathwayColors
   if (ncol(datam)>1 & nrow(datam)>1){
             heatmap.2(datam, margins = heatmapMargins,  keysize=heatmapKeysize, col=colv,main=maintitle,key.title=keytitle,key.xlab="",key.ylab="",cexCol=1)} else {
			 warning("there is only one dataset/comparison or one gene/pathway detected and heatmap figure is not generated")
			 kable(datam,caption=maintitle,col.names=paste0(colnames(datam),"(",keytitle,")"))
			 }
 }
...
if(hasDiffPathway){
  displayDiffPathway<-DISPLAY_FIGURE 
}else{
  displayDiffPathway<-FALSE  
}
...
A short description of Table 1 metrics is provided below: 
  ...
  * _**EID**_ - Unique Name of the experiment
