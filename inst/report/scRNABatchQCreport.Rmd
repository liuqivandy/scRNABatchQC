...
plotHeatmap<-function(datam,feature=c("hvg","hvgPath","PCg","PCgPath","diffg","diffgPath")){
  ...
  if (ncol(datam)>1 & nrow(datam)>1){
    ...
    warning("there is only one dataset/comparison or one gene/pathway detected and heatmap figure is not generated")
	  kable(datam,caption=tabRef(feature, maintitle),col.names=paste0(colnames(datam),"(",keytitle,")"))
	}
}
...
if(hasPathway){
  displayPathway<-DISPLAY_FIGURE 
}else{
  displayPathway<-FALSE  
}

hasPC1Pathway<-!is.null(pc1Pathways)
if(hasPC1Pathway){
  displayPC1Pathway<-DISPLAY_FIGURE 
}else{
  displayPC1Pathway<-FALSE  
}

hasDiffGenes<-!is.null(plotData$scesMerge@metadata$diffFC$genes)
displayDiffGenes<-DISPLAY_FIGURE & hasDiffGenes
if(hasDiffGenes){
  drawDiffGenesHeatmap<-DISPLAY_FIGURE & ncol(plotData$scesMerge@metadata$diffFC$genes)>1 & nrow(plotData$scesMerge@metadata$diffFC$genes)>1
  drawDiffGenesTable<-!drawDiffGenesHeatmap
}else{
  drawDiffGenesHeatmap<-FALSE
  drawDiffGenesTable<-FALSE
}

hasDiffPathway<-!is.null(plotData$scesMerge@metadata$diffFC$pathways)
if(hasDiffPathway){
  drawDiffPathwayHeatmap<-DISPLAY_FIGURE & ncol(plotData$scesMerge@metadata$diffFC$pathways)>1 & nrow(plotData$scesMerge@metadata$diffFC$pathways)>1
  drawDiffPathwayTable<-!drawDiffPathwayHeatmap
}else{
  drawDiffPathwayHeatmap<-FALSE 
  drawDiffPathwayTable<-FALSE
}
...
```{r density_total_counts, eval=DISPLAY_FIGURE, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.align="center", fig.cap=figRef("density_total_counts", "The total number of counts"), fig.height=4}
...
```{r plotRibosomalRNA, eval=DISPLAY_FIGURE, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.align="center", fig.cap=figRef("plotRibosomalRNA", "The proportion of reads mapped to ribosomal-proteins-encoded genes"), fig.height=4}
...
```{r plotVariancerRNA, eval=DISPLAY_FIGURE, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.align="center", fig.cap=figRef("plotVariancerRNA", "The percentage of expression variances explained by the proportion of ribosomal proteins reads"), fig.height=4}
...
```{r child="scRNABatchQCreport_biological.Rmd",  eval=DISPLAY_FIGURE, echo=FALSE}
```
...
The PCA Plot for the first two components (`r figRef("plotPCA")`) shows the similarities between cells within or across scRNA-seq experiments.
...
The tSNE plot for the first two components (`r figRef("plottSNE")`) shows the similarities between cells within or across scRNA-seq experiments. tSNE is a non-linear dimension reduction technique, which PCA reduce the dimension through linear transformation. 
...
**Note: If no pathways results are included for HVGs, PC-related genes or differentially expressed genes. There are two possible reasons. One is that pathways are not enriched. The other reason is that gene name is not gene symbol or organism is not supported by WebGestalt. WebGestalt supports 12 organisms, which includes athaliana, btaurus, celegans, cfamiliaris, drerio, sscrofa, dmelanogaster, ggallus, hsapiens, mmusculus, rnorvegicus, scerevisiae.
