...
# Quick start
To easiest way to generate a summary QC report is to run scRNABatchQC.  The only required input is a vector of file names (or a URL starting http://, file://, etc) for gene-cell count matrices, or a vector of path names from 10X CellRanger >=3.0, or a vector of SingleCellExperiment or Seurat objects.
...
The parameter inputs can be a string vector of file names containing gene-cell count matrices, or a vector of path names containing the barcodes.tsv.gz, features.tsv.gz, and matrix.mtx.gz provided by 10X from CellRanger >=3.0, or a vector of SingleCellExperiment or Seurat objects.
