...
# Output data
The scRNABatchQC generates two types of output. One is a HTML file, a QC report providing both metrics and diagnostic graphics to assess the similarity/difference on a number of technical and biological features for the input datasets. By default, the HTML report will be written in your working directory named report.html.  The other is a list of SingleCellExperiment objects containing raw, filtered and normalized data, quality meta data from each dataset and the merged dataset. 
...
scRNABatchQC not only generates a QC summary report in a HTML file, but only return a list of SingleCellExperiment objects with raw count, normalized data, and QC metadata for downstream analysis. 
