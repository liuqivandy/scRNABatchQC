...
After installing scRNABatchQC, use following codes to run examples
	...
	# one dataset has more than 14,000 cells and 2,4904 genes, thus big memory (>=16Gb) is required.
	#a report named "report.html" will be generated in your working directory
	#One SingleCellExperiment object (scesMerge) containing the combined dataset and a list of SingleCellExperiment objects (sces, each contains the preprocessed dataset and metadata for one dataset) will be returned.
